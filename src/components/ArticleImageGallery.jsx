...
export default function ArticleImageGallery({
  ...
  return (
    <div className="mb-3">
      <div className="mb-3 border-y border-[#ddd3bf] py-2">
        <div className="mb-1.5 flex items-center justify-between gap-3">
          <div className="flex shrink-0 items-center pl-0.5">
            {safeContributors.slice(0, 4).map((s, index) => (
              <span
                key={s.id}
                className="relative inline-flex h-8 w-8 items-center justify-center overflow-hidden rounded-full border border-[#b8b39f] bg-[#f1ecde]"
                style={{
              ...
              >
                {s.avatar_url ? (
                  <img
                    src={resolveAvatarUrl(s.avatar_url)}
                    alt={s.username || "Kontributor"}
                    className="h-full w-full object-cover"
                    loading="lazy"
                  />
                ) : (
                  <span className="text-[0.7rem] font-semibold text-[#7f7a68]">
                    {(s.username || "K").slice(0, 1).toUpperCase()}
                  </span>
                )}
              </span>
            ))}
        ...
        <div className="flex items-center justify-between">
          <div className="relative" ref={dropdownRef}>
            <button
              className="flex items-center p-0 text-[0.95rem] italic text-[#2f80e7] hover:underline"
            ...
            {showContributors && safeContributors.length > 0 && (
              <div className="absolute left-0 z-50 mt-1 min-w-[200px] rounded border bg-white py-2 shadow-sm">
                {safeContributors.map((contributor) => (
                  <a
                    key={contributor.id}
                    href={`/contribution/${contributor.username}`}
                    className="flex items-center gap-2 px-3 py-2 text-gray-700 hover:bg-gray-100"
                  >
                    <span className="inline-flex h-6 w-6 items-center justify-center overflow-hidden rounded-full border border-[#d4cfbf] bg-[#f2eee1]">
                      {contributor.avatar_url ? (
                        <img
                          src={resolveAvatarUrl(contributor.avatar_url)}
                          alt={contributor.username || "Kontributor"}
                          className="h-full w-full object-cover"
                        />
                      ) : (
                        <span className="text-[0.65rem] font-semibold text-[#7f7a68]">
                          {(contributor.username || "K")
                            .slice(0, 1)
                            .toUpperCase()}
                        </span>
                      )}
          ...
          <span className="text-[0.95rem] italic text-[#2f80e7]">Sebarkan</span>
        </div>
      </div>
      ...
      {(images.length > 0 || (canEdit && !isDiskursus)) && (
        <>
          <div
            id="article-image-gallery-strip"
            className="article-gallery-strip flex gap-3 overflow-x-auto whitespace-nowrap rounded-none p-0"
          ...
          >
            {previewImages.map((img) => (
              <div key={img.id} className="relative shrink-0">
                <img
                  ...
                  alt={img.caption || ""}
                  className={`h-[92px] w-[132px] rounded-[3px] object-cover shadow-sm sm:h-28 sm:w-40 ${img.featured ? "border-2 border-emerald-500" : "border border-[#d8d2c2]"}`}
                />
                {canEdit && !isDiskursus && (
                  <div
            ...
            {canEdit && !isDiskursus && (
              <div
                ...
                className="flex h-[92px] w-[132px] shrink-0 cursor-pointer items-center justify-center rounded-[3px] border border-dashed border-[#cfc8b4] text-gray-500 sm:h-28 sm:w-40"
              >
                + Add
              </div>
            )}
          ...
          <div className="mt-1 flex items-center justify-between gap-2 text-[0.78rem] italic text-[#2f80e7] sm:text-[0.86rem]">
            {canEdit && !isDiskursus ? (
              <button
            ...
            <button
              ...
              onClick={() => {
                const galleryStart = document.getElementById(
                  "article-image-gallery-strip",
                );
                if (galleryStart) {
